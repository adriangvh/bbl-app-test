...
const COMPANY_SELECT =
  "id,name,company_group,organization_number,organization_type,responsible_partner,audit_stage,overall_risk_assessed,fraud_risk_documented,controls_tested,partner_review_ready,task_due_date,signing_document";
...
function toCompanyRow(company) {
  return {
    ...
    signing_document: company.signingDocument ?? null,
  };
}
...
function fromCompanyRow(row) {
  return {
    ...
    signingDocument: row.signing_document || "",
  };
}
...
export async function updateCompanySigningDocument(
  companyId,
  actorId,
  actorRole,
  content
  ...
  if (normalizedRole !== "partner") {
    return {
      ok: false,
      status: 403,
      error: "Only partners can edit signing documents.",
    };
  }

  if (typeof content !== "string") {
    return { ok: false, status: 400, error: "Signing document content must be text." };
  }
  if (content.length > 40000) {
    return { ok: false, status: 400, error: "Signing document is too large." };
  }
  ...
  const rows = await restRequest("audit_companies", {
    method: "PATCH",
    filters: [{ column: "id", op: "eq", value: companyId }],
    body: { signing_document: content },
    prefer: "return=representation",
  });

  if (!Array.isArray(rows) || rows.length === 0) {
    return { ok: false, status: 404, error: "Company not found.", lock: activeLock };
  }
  ...
  await logActivity(
    ...
    activeLock.actorName,
    "signing_document",
    `Signing document updated (${content.length} chars).`
  );
...
export async function getDashboardSummary() {
  ...
  const [companyRows, taskRows, lockMap, activityRows] = await Promise.all([
    ...
    restRequest("audit_activity_events", {
      method: "GET",
      select: "company_id,event_type,message,created_at",
  ...
  const stageTimelineBuckets = dayBuckets.map((bucket) => ({
    date: bucket.date,
    stages: auditStages.reduce((acc, stage) => ({ ...acc, [stage]: 0 }), {}),
  }));
  const stageTimelineByDate = new Map(stageTimelineBuckets.map((bucket) => [bucket.date, bucket]));

  (activityRows || []).forEach((row) => {
    const date = String(row.created_at || "").slice(0, 10);
    const bucket = bucketByDate.get(date);
    const stageBucket = stageTimelineByDate.get(date);
    if (!bucket) {
      return;
    }
    if (row.event_type === "stage_change") {
      ...
      const match = String(row.message || "").match(/to "([^"]+)"/i);
      const toStage = match ? match[1] : "";
      if (stageBucket && auditStages.includes(toStage)) {
        stageBucket.stages[toStage] += 1;
      }
    } else if (row.event_type === "stage_signing") {
      bucket.sentToSigning += 1;
      bucket.processedTotal += 1;
      if (stageBucket) {
        stageBucket.stages.Signing += 1;
      }
    }
  });
  ...
  return {
    ...
    stageTimeline: stageTimelineBuckets,
  };
}
