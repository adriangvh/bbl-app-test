...
function buildViewerNameKeys(name) {
  ...
  return Array.from(new Set([key, firstToken].filter(Boolean)));
}
...
export async function addTaskDiscussionComment(
  ...
  const rows = await restRequest("audit_task_discussions", {
    ...
    body: {
      ...
      author_name: actorName || activeLock.actorName || "Unknown user",
      message: trimmed,
    },
  });
  ...
  await logActivity(
    companyId,
    actorId,
    actorName || activeLock.actorName,
    "task_discussion",
  ...
  const mentionKeys = extractMentionTokens(trimmed).filter(
    (key) => key !== normalizeNameKey(actorName)
  );
    ...
    try {
      await restRequest("audit_notifications", {
        ...
        body: mentionKeys.map((mentionKey) => ({
          company_id: companyId,
          task_id: taskId,
          recipient_name: mentionKey,
          recipient_name_key: mentionKey,
          sender_name: actorName || activeLock.actorName || "Unknown user",
  ...
  return { ok: true, comment: fromDiscussionRow(inserted), lock: activeLock };
}
